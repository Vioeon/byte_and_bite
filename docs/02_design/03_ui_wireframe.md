...
- 출력 데이터 항목 (Output Data):
    ...
    - 상단 영역: 카테고리 및 식당 이름 검색어 입력, 리뷰작성
...
- 출력 데이터 항목 (Output Data):
    - GNB(Global Navigation Bar) 영역 : 서비스 타이틀, 로그아웃, 맛집리뷰, 맛집소식, 마이페이지
    - 상단 영역: 카테고리 및 식당이름 검색어 입력, 소식 작성
...
- 입력 데이터 및 검증 규칙 (Input Data & Validation):
    - 닉네임 : 공백제출 제한, 2~10자 이하
- 화면 제어 및 권한 규칙 (Behavior Rules):
    - 비로그인 사용자가 접근 시 로그인 페이지(`/login`)로 강제 리다이렉트 처리
    - 세션에 저장된 사용자 정보를 기반으로 프로필 정보 및 작성한 리뷰 목록 자동 바인딩
    - 일반 회원(`USER`) 권한으로 접속 시 맛집 운영자 전용 식당 정보 카드는 화면에서 숨김 처리(Thymeleaf`th:if` 분기)
...
- 화면 제어 및 권한 규칙 (Behavior Rules):
    - 비로그인 사용자가 접근 시 로그인 페이지(`/login`)로 강제 리다이렉트 처리
    - 로그인된 사용자의 권한(`Role`)에 따라 마이페이지 내부 컴포넌트 동적 노출 (Thymeleaf `th:if` 분기)
    - 세션에 저장된 사용자 정보를 기반으로 프로필 정보 및 작성한 리뷰 목록 자동 바인딩
    - 세션 내 `role == 'MANAGER'`인 경우에만 **'맛집 운영자 식당 정보 카드'** 및 **[식당 정보 수정]** 버튼 활성화 및 노출 (일반 회원 `USER` 권한인 경우 해당 카드 영역 숨김 처리)
    - 닉네임 변경 버튼 클릭 시 닉네임 변경 창
...
- 입력 데이터 및 검증 규칙 (Input Data & Validation):
    ...
    - 비밀번호 : 필수 입력 항목, 공백 제출 제한, 8 ~ 20자 이하
- 화면 제어 및 권한 규칙 (Behavior Rules):
