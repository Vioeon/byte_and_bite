...
- 화면 제어 및 권한 규칙 (Behavior Rules):
    ...
    - 비로그인 상태
        - 맛집 리뷰 목록 조회 가능
        - 맛집 소식 목록 조회 가능
        - 리뷰 카드, 맛집 소식 카드, 마이페이지 접근 시 HttpSession 검증 후 로그인 페이지 이동
...
- 화면 제어 및 권한 규칙 (Behavior Rules):
    ...
    - 비회원은 맛집 리뷰 목록 및 맛집 소식 목록 조회만 가능하며, 맛집 리뷰 카드, 맛집 소식 카드, 마이페이지 등 회원 전용 기능 접근 시 HttpSession 검증을 통해 로그인 페이지로 이동한다.
